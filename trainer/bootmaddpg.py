from trainer.network import Critic_MA, Actor_MA, EnsembleCrt_MA, EnsembleAct_MA
import torch
...
class BootMADDPGa:
    def __init__(self, dim_obs, dim_act, n_agents, args):
        ...
        self.n_ensemble=args.n_ensemble  
        self.critics = []
        self.actors = [EnsembleAct_MA(dim_obs, dim_act,self.n_ensemble) for _ in range(n_agents)]
    ...
    def update(self,i_episode):
        ...
        for agent in range(self.n_agents):
            ...
            state_i = state_batch[:, agent, :]
            action_i = self.actors[agent](state_i)
            ac = torch.stack([action_batch.clone() for _ in range(self.n_ensemble)]).type(FloatTensor)
            for i in range(self.n_ensemble):
                ac[i,:, agent, :] = action_i[i]
            whole_action = torch.stack([ac[i].view(self.batch_size, -1) for i in range(self.n_ensemble)]).type(FloatTensor)
    ...
    def choose_action(self, state, noisy=True):
        ...
        for i in range(self.n_agents):
            sb = obs[i].detach()
            act = self.actors[i](sb.unsqueeze(0),self.epi_actors[i]).squeeze()
            if noisy:
                act += self.noise_scale*torch.from_numpy(np.random.randn(self.n_actions) * self.var[i]).type(FloatTensor)
